...
object SimpleList {
  ...
  def appendWithFoldRight[A](a1: SimpleList[A], a2: SimpleList[A]): SimpleList[A] = foldRight(a1, a2)((a, b) => Cons(a, b))

  def appendWithFoldLeft[A](a1: SimpleList[A], a2: SimpleList[A]): SimpleList[A] = foldLeft(SimpleList.reverse(a1), a2)((b, a) => Cons(a, b))
  ...
  def filter[A](as: SimpleList[A])(f: A => Boolean) = foldRight(as, Nil: SimpleList[A])((a, acc) => if (f(a)) Cons(a, acc) else acc)

  // 3.20
  def flatMap[A, B](as: SimpleList[A])(f: A => SimpleList[B]): SimpleList[B] = foldRight(as, Nil: SimpleList[B])((a, acc) => append(f(a), acc))
